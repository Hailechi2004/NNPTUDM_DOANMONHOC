...
function calculateCartPricing(part, promotion) {
  ...
  const defaultState = partController.getPromotionState(part.price, part.discountPrice, part.hasPromotion);
  ...
  const promoPrice = partController.calculatePromotionPrice(defaultPrice, promotion);
...
async function getShopProductDetail(id, authUser = null) {
  ...
  const assignedPromotion = part.promotionId ? await partController.findPromotionForPart(id) : null;
...
async function applyPromotionCode(authUser, partId, promoCode) {
  ...
  const promotion = await partController.findPromotionForPart(partId);
